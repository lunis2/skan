...
import {validateInn} from "../../services/validateINN";
import PubServ from "../../services/PubServ";
...
export default function SearchWidget() {
    ...
    async function onSearchBtnClick() {
        ...
        await PubServ.getHistograms(inn, tonality, limit,
        ...
        await PubServ.getPublicationsList(inn, tonality, limit,
            selectedStartDate, selectedEndDate, checkMaxFullnessState,
            checkInBusinessNewsState, checkOnlyMainRoleState, checkExcludeAnnouncementsState)
            .then(response => {
        ...
        <section className={styles.parent}>
            <div className={styles.wrapper}>
                <div className={styles.inputs}>
                    ...
                    <input id="innNumber" className={isInnError ? styles.input__error : ""} onBlur={onInnInputBlur}
                           onChange={onInnInputChange} type="text" placeholder="10 цифр" maxLength={10}/>
                    ...
                    <span>Количество документов к выдаче<sup
                        className={isDocsCountError ? styles.sup__error : ""}>*</sup></span>
                    <input ref={docsCountRef} id="docsCount" className={isDocsCountError ? styles.input__error : ""}
                           onChange={onDocsCountInputChange} onBlur={onDocsCountInputBlur} type="number"
                           placeholder="От 1 до 1000" min="1" max="1000"/>
                    {isDocsCountError &&
                        <span className={styles.error_label} target="docsCount">Обязательное поле</span>}
                    ...
                    <div className={styles.range_inputs}>
                        <div className={styles.range_inputs__wrapper}>
                            <DateRangeInputs selectedStartDate={selectedStartDate} selectedEndDate={selectedEndDate}
                                             setSelectedStartDate={setSelectedStartDate}
                                             setSelectedEndDate={setSelectedEndDate} isError={isDateRangeError}
                                             setError={setDateRangeError}/>
                        </div>
                        {isDateRangeError &&
                            <span className={styles.error_label} target="dateRange">Введите корректные данные</span>}
                    </div>
                </div>

                <div className={styles.checkboxes}>
                    <div className={styles.checkbox__wrapper}>
                        <input type="checkbox" id="1" name="1" checked={checkMaxFullnessState}
                               onChange={event => setCheckMaxFullnessState(event.currentTarget.checked)}/>
                        <label htmlFor="1">Признак максимальной полноты</label>
                    </div>

                    <div className={styles.checkbox__wrapper}>
                        <input type="checkbox" id="2" name="2" checked={checkInBusinessNewsState}
                               onChange={event => setCheckInBusinessNewsState(event.currentTarget.checked)}/>
                        <label htmlFor="2">Упоминания в бизнес-контексте</label>
                    </div>

                    <div className={styles.checkbox__wrapper}>
                        <input type="checkbox" id="3" name="3" checked={checkOnlyMainRoleState}
                               onChange={event => setCheckOnlyMainRoleState(event.currentTarget.checked)}/>
                        <label htmlFor="3">Главная роль в публикации</label>
                    </div>

                    <div className={styles.checkbox__wrapper}>
                        <input type="checkbox" id="4" name="4" checked={check4State}
                               onChange={event => setCheck4State(event.currentTarget.checked)} disabled/>
                        <label htmlFor="4">Публикации только с риск-факторами</label>
                    </div>

                    <div className={styles.checkbox__wrapper}>
                        <input type="checkbox" id="5" name="5" checked={check5State}
                               onChange={event => setCheck5State(event.currentTarget.checked)} disabled/>
                        <label htmlFor="5">Включать технические новости рынков</label>
                    </div>

                    <div className={styles.checkbox__wrapper}>
                        <input type="checkbox" id="6" name="6" checked={checkExcludeAnnouncementsState}
                               onChange={event => setCheckExcludeAnnouncementsState(event.currentTarget.checked)}/>
                        <label htmlFor="6">Включать анонсы и календари</label>
                    </div>

                    <div className={styles.checkbox__wrapper}>
                        <input type="checkbox" id="7" name="7" checked={check7State}
                               onChange={event => setCheck7State(event.currentTarget.checked)} disabled/>
                        <label htmlFor="7">Включать сводки новостей</label>
                    </div>
                </div>

                <div className={styles.button_wrapper}>
                    <button className={styles.search_button} ref={searchBtnRef} onClick={onSearchBtnClick}
                            disabled={!isCompleted}>Поиск
                    </button>
